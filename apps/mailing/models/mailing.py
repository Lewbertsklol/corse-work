...
class Mailing(AbstractModel):
    ...
    def name(self):
        '''
        Имя задачи в django_celery_beat должно быть уникальным. 
        Поэтому при создании в начале имени рассылки добавляем email пользователя + ::
        А для визуального отображения, часть с имейлом обрезаем
        '''
